...
class ApartmentController {
  ...
  async getAll(req, res, next) {
    try {
      let { page, limit, location, leavingRoom, rentalPeriod, userId, premium } = req.query;
      page = Number(page) || 1;
  ...
  async changePremiumStatus(req, res, next) {
    try {
      ...
      const apartment = await Apartment.update({premium}, {
        where: {
            id
          },
        })
    ...
    try {
      ...
      for ( let i = 0; i < photos.length; i++) {
        const path = path.resolve(
          __dirname, '..', 'static', photos[i].imageName
        )
        fs.unlinkSync(path)
      }
